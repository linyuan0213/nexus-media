...
- 媒体类型支持「动漫」：RSS/站点搜索等**批量识别**按 TMDB genre16 判定为动漫（此前仅单条识别具备）；关键词「动漫」映射为动漫类型；订阅/搜索全链路透传，下载与入库均按动漫目录处理
- 订阅 **TMDB 别名匹配**：订阅名与外部榜单名称不一致时（如 BGM「画完这个再去死」= 订阅「描绘直至生命尽头」）也能识别为已订阅，别名映射缓存 30 分钟
- 媒体库**实时存在性检查**：同步登记薄为空/未命中时回退实时查询媒体服务器（5 分钟缓存），使「媒体库里有但未订阅」也能显示已入库

### 修复

- 媒体类型不再被错误降级或丢弃：TMDB `media_type` 字符串归一化，避免类型被写成字符串而跳过动漫判定；搜索结果 UNKNOWN 不再误标为动漫
- 订阅状态判定优先级修正：**订阅中（已订阅）优先于已入库**；订阅完成（历史记录）视为已入库；历史订阅缺 `media_type` 时按 TMDB genre 推断，修复「全是动漫却部分显示电视剧」
- TMDB 缓存过期时回源识别，避免订阅被批量标记为错误状态（E）
- 刷流：免费到期暂停的种子「再次免费自动启动」受活跃星期/时段约束，非活跃时段不再拉起
- 手动转移：TMDB 类型选错时自动回退另一类型并纠正，修复「识别失败，无法查询到TMDB信息」
- 数据库：`SUBSCRIBE_TV_EPISODES.EPISODES`、`SUBSCRIBE_TVS/SUBSCRIBE_MOVIES.RSS_SITES/SEARCH_SITES` 由 varchar(255) 加宽为 TEXT，修复新增长集数订阅报 (1406) `Data too long for column 'EPISODES'`（迁移 `e1f2a3b4c5d6`，SQLite/MySQL/PG 均已验证，降级前截断超长值）

### 其他

- 合并上游 Jellyfin 12 认证兼容（改用 `Authorization: MediaBrowser Token` 头）并补充单元测试

## v4.20.20 (2026-09-18)

### 修复

- 订阅媒体类型保真：`type=anime` 不再被强制降级为电视剧；订阅新增时把 `media_type` 写入 NOTE 并在查询/匹配链路透传，动漫订阅的下载与入库均按动漫目录处理
- 匹配阶段：已识别为动漫的种子不再因订阅属于电视剧表而降级
- 修复 RSS 状态映射把动漫当作电影的问题（改用 `MediaTypeMapper.to_tmdb`）；删除订阅时动漫按电视剧表处理

## v4.20.19 (2026-09-16)

### 新增

- 签到插件新增「登录模式」：仅访问站点首页刷新登录态、不执行签到；支持 HTTP 与浏览器自动化（`login_sites` / `login_browser_sites`）；登录模式站点每次调度都会刷新，插件版本 1.0 → 1.1

### 修复

- 种子属性（torrent_attr）抓取：
  - HTML 详情页识别登录重定向与未登录状态，cookie 失效时明确报错，不再把登录页静默当作「确定非免费」并缓存
  - 透传站点「浏览器自动化」开关（`chrome` / `browser_persistent`），Cloudflare 挑战站点（如 OurBits）可走浏览器过盾
  - 详情页抓取合并站点维护的 `headers`（此前仅发送 UA，自定义头丢失）
