...
class TestSchedulerServiceRemoveJob:
    """测试 SchedulerService.remove_job"""

    @patch("app.scheduler_service.BackgroundScheduler")
    @patch("app.scheduler_service.Config")
    def test_remove_job_success(self, mock_config_cls, mock_scheduler_cls):
        from app.scheduler_service import SchedulerService
        # 重置单例缓存，确保每次拿到新实例
        SchedulerService._instances.pop(SchedulerService, None)
