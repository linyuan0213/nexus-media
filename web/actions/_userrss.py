...
class WebActionUserrssMixin:
    ...
    def _get_userrss_task(data):
        ...
        """
        taskid = data.get("id")
        return WebActionBase._success(detail=RssChecker().get_rsstask_info(taskid=taskid))
    ...
    def _check_userrss_task(data):
        ...
        try:
            ...
            return WebActionBase._fail(msg="自定义订阅状态设置失败")
    ...
    def _get_rssparser(data):
        ...
        """
        pid = data.get("id")
        return WebActionBase._success(detail=RssChecker().get_userrss_parser(pid=pid))
    ...
    def _list_rss_articles(data):
        ...
        count = len(articles)
        if articles:
            return WebActionBase._success(data=articles, count=count, uses=uses, address_count=address_count)
        else:
            return WebActionBase._fail(msg="未获取到报文")

    def _rss_article_test(self, data):
        ...
        return self._success(data=media_dict)
    ...
    def _list_rss_history(data):
        ...
        if downloads:
            return WebActionBase._success(data=downloads, count=count)
        else:
            return WebActionBase._fail(msg="无下载记录")

    @staticmethod
    def _rss_articles_check(data):
        if not data.get("articles"):
            return WebActionBase._fail(code=2)
