...
class WebActionFilterMixin:
    ...
    def get_filterrules():
        ...
        with open(sql_file, "r", encoding="utf-8") as f:
            ...
            while i < len(sql_list):
                ...
                i = i + 2
        return WebActionBase._success(ruleGroups=RuleGroups, initRules=Init_RuleGroups)
    ...
    def _add_filtergroup(data):
        ...
        if not name:
            return WebActionBase._fail(code=-1)
    ...
    def _filterrule_detail(data):
        ...
        return WebActionBase._success(info=ruleinfo)
    ...
    def _share_filtergroup(data):
        ...
        if not group_info:
            return WebActionBase._fail(msg="规则组不存在")
        group_rules = _filter.get_filter_rule(groupid=gid)
        if not group_rules:
            return WebActionBase._fail(msg="规则组没有对应规则")
        rules = []
        ...
        return WebActionBase._success(string=json_string)
    ...
    def _import_filtergroup(data):
        ...
        try:
            ...
            ExceptionUtils.exception_traceback(err)
            return WebActionBase._fail(msg="数据格式不正确，%s" % str(err))
    ...
    def _rule_test(data):
        ...
        return WebActionBase._success(flag=match_flag, text="匹配" if match_flag else "未匹配", order=100 - res_order if res_order else 0)
