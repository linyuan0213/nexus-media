...
function media_search(tmdbid, title, type) {
  ...
  ajax_post("/api/system/search", param, function (ret) {
...
function check_system_online() {
  ajax_post("/api/system/refresh_process", {type: "restart"}, function (ret) {
    if (ret.code === -1) {
      logout();
    } else {
...
//注销
function logout() {
  ajax_post("/api/system/logout", {}, function (ret) {
...
function restart() {
  show_confirm_modal("立即重启系统？", function () {
    hide_confirm_modal();
    ajax_post("/api/system/restart", {}, function (ret) {
    }, true, false);
...
function show_mediainfo_modal(rtype, name, year, mediaid, page, rssid) {
  ...
  ajax_post("/api/media/media_info", {
    "id": mediaid,
    "title": name,
    "year": year,
    "type": rtype,
...
function add_rss_media(name, year, type, mediaid, page, season, func) {
  ...
  ajax_post("/api/rss/add_rss_media", data, function (ret) {
    if (ret.code === 0) {
      if (ret.page) {
        navmenu(ret.page);
      } else {
...
function remove_rss_media(name, year, type, rssid, page, tmdbid, func) {
  ...
  ajax_post("/api/rss/remove_rss_media", data, function (ret) {
    if (func) {
      func();
    } else if (ret.page) {
...
function refresh_rss_media(type, rssid, page) {
  hide_mediainfo_modal();
  ajax_post("/api/rss/refresh_rss", {"type": type, "rssid": rssid, "page": page}, function (ret) {
    if (ret.page) {
      window_history_refresh();
    } else {
...
function show_default_rss_setting_modal(mtype) {
  ...
  // 查询已保存配置
  ajax_post("/api/rss/get_default_rss_setting", {mtype: mtype}, function (ret) {
...
function save_default_rss_setting() {
  ...
  ajax_post("/api/system/set_system_config", {key: key, value: value}, function (ret) {
    if (ret.code === 0) {
...
function show_edit_rss_media_modal(rssid, type) {
  ...
  // 获取订阅信息
  ajax_post("/api/rss/rss_detail", {"rssid": rssid, "rsstype": type}, function (ret) {
    if (ret.code === 0) {
...
function refresh_filter_select(obj_id, aync = true) {
  ajax_post("/api/filter/get_filterrules", {}, function (ret) {
    if (ret.code === 0) {
...
function refresh_rsssites_select(obj_id, item_name, aync = true) {
  ajax_post("/api/site/get_sites", {rss: true, basic: true}, function (ret) {
    if (ret.code === 0) {
...
function refresh_searchsites_select(obj_id, item_name, aync = true) {
  ajax_post("/api/download/get_indexers", {check: true, basic: true}, function (ret) {
    if (ret.code === 0) {
...
function refresh_savepath_select(obj_id, aync = true, sid = "", is_default = false, site = "") {
  ...
  if (!sid && !is_default && !site) {
    ...
    ajax_post("/api/download/get_download_dirs", {sid: sid, site: site}, function (ret) {
      if (ret.code === 0) {
...
function refresh_downloadsetting_select(obj_id, aync = true, is_default = false) {
  ...
  ajax_post("/api/download/get_download_setting", {}, function (ret) {
    if (ret.code === 0) {
...
function download_link() {
  ...
  ajax_post("/api/download/download", {"id": id, "dir": dir, "setting": setting}, function (ret) {
    if (ret.code === 0) {
...
function media_name_test(name, result_div, func, subtitle) {
  ...
  ajax_post("/api/media/name_test", {"name": name, "subtitle": subtitle || ""}, function (ret) {
    if (func) {
      func();
    }
    if (ret.code === 0) {
...
function re_identification(flag, ids) {
  ajax_post("/api/sync/re_identification", {"flag": flag, "ids": ids}, function (ret) {
    if (ret.code == 0) {
      navmenu(flag);
    } else {
...
function manual_media_transfer() {
  ...
  let url = (manual_type === '3') ? "/api/sync/rename_udf" : "/api/sync/rename"
...
function search_tmdbid_by_name(keyid, resultid) {
  ...
  ajax_post("/api/media/search_media_infos", {"keyword": name, "searchtype": "tmdb"}, function (ret) {
