...
class ChromeSigninSimulator:
    def __init__(self, site_conf=None, site_engine=None):
    ...
    def signin(self, site_info: dict, plugin_ctx) -> str:
        ...
        if not site_url or not server_url:
            return ""
        ...
        try:
            with BrowserSession(
                ...
                result = session.navigate(home_url, cookie=site_cookie)
                ...
                session.click(f"xpath:{xpath_str}")
                time.sleep(15)
                ...
                if re.search(r"已签|签到已得|签到成功|签到.*成功|获得.*积分|签到.*积分", html_text, re.IGNORECASE):
                    ...
                    plugin_ctx.info(f"{site} 今日已签到")
                    return f"[{site}]今日已签到"
                elif re.search(r"错误|失败|异常|error|fail", html_text, re.IGNORECASE):
