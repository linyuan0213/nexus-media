...
class SITEUSERINFOSTATS(Base):
    ...
    USER_LEVEL: Mapped[str] = mapped_column(String(255))
    JOIN_AT: Mapped[str] = mapped_column(String(255), default="")
    UPDATE_AT: Mapped[str] = mapped_column(String(255), default="")
    ...
    SEEDING: Mapped[int] = mapped_column(Integer, default=0)
    LEECHING: Mapped[int] = mapped_column(Integer, default=0)
    SEEDING_SIZE: Mapped[int] = mapped_column(BigInteger, default=0)
    BONUS: Mapped[float] = mapped_column(Float, default=0.0)
    URL: Mapped[str] = mapped_column(String(512), unique=True)
    MSG_UNREAD: Mapped[int] = mapped_column(Integer, default=0)
    EXT_INFO: Mapped[str] = mapped_column(String(255), default="")
